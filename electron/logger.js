...
const customFormat = combine(errors({ stack: true }), // Log stack traces for errors
timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS' }), json());
const transports = [];
...
if (isDevelopment) {
    transports.push(new winston.transports.Console({
        level: logLevel,
        format: combine(timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS' }), winston.format.colorize(), printf(({ level, message, timestamp, category, processType, stack, ...metadata }) => {
            ...
            if (stack) {
                ...
                try {
                    log += ` ${JSON.stringify(metadata, (key, value) => typeof value === 'bigint' ? value.toString() : value // BigInt serialization
                    )}`;
                }
...
export function logFromRenderer(level, category, message, metadata) {
